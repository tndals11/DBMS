...
# 2. ERD 구성요소
# 1) Entity
-- 존재하는 실체 (테이터베이스에서 관리되어야 하는 대상)
-- 고유하게 식별 가능해야 함 (PK)
-- 사각형 (엔티티명은 사각형 안에 명시)

# 2) 속성
-- 엔티티의 세부정보, 설명 항목
-- 일반 속성(이름, 주소, 전화번호 등), 기본키(PK: 회원 ID, 학번 등), 외래키(FK: 주문 테이블의 고객ID)
-- 타원형 (속성명은 원 안에 명시)	
...
# 1:N(일대다) - 한 엔티티가 여러 개와 관계 (교수 - 강의, 학생 - 수강, 게시글 - 댓글)
...
# 즐겨찾기 프로그램 테이블) 고유번호 ID PK, 회원 번호 FK, 프로그램명 ... 등
