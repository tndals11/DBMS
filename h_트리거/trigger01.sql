...
-- 데이터 무결성: 수동으로 처리할 작업을 자동으로 처리 데이터 오류 방지 (주문 후 결제 데이블 자동 삽입) 
...
# 5. 트리거 실무 사용법 # 
# : 특정 테이블에서 이벤트(inset, update, delete) 발생 시 자동 '로그 기록'
...
# cf) 원자성(Atomicitiy): 모두 성공 또는 모두 실패 (하나라도 실패 시 모두 롤백)
