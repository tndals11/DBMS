...
# : CREATE(생성), ALTER(수정), DROP(삭제), TRUNCATE(데이터삭제)
...
/*
    ...
    cf) 테이블명 뒤 열 이름의 나열을 생략할 경우
		: values 뒤의 값 순서는 테이블 정의시 작성한 열의 순서 & 개수와 동일
		>> name(문자), age(숫자), major(문자)순alter
        
        cf) 전체 테이블의 컬럼 순서 & 개수와 차이가 나는 경우 반드시! 원고자하는 열 이름 나열!
*/
...
# 맨 아래 Safe Updates 옵션 해체(WHERE 조건 제약 X) / 체크(WHERE 조건 제약 o)
# +) 재시작해야 적용된다.


select * from `example02`;

# cf) 실행중인 세션에서 일시적으로 Safe 모드 해체 
SET SQL_SAFE_UPDATES=1; # 1: 모드 사용
SET SQL_SAFE_UPDATES=0; # 0: 모드 해체
