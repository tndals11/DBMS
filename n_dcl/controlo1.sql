...
# '계정명'@'접속범위설정' IDENTIFIED BY '비밀번호';

# cf) 접속 범위 설정
# - 'localhost': 현 시스템 컴퓨터 (MySQL 서버가 설치된 이 컴퓨터에서만 로그인 가능)
# - '%': 어떤 IP든지 접속 가능을 의미 (보안상 위험!, 실습 외에는 권장하지 않음) 
# - '127.0.0.1': 해당 IP 주소를 가진 컴퓨터에서만 로그인 가능 (>> 127.0.0.1: lcalhost와 동일)
...
# 2) 권한을 모두 회수 뒤에도 USAGE만 남음

# CLI(명령어)로 로그인하는 방법 - 명령 프롬프트 창 
# mysql -u 사용자이름 -p -h localhost;
...
# 2) 모든 권한 제거
REVOKE ALL PRIVILEGES, GRANT OPTION FROM 'readonly_user'@'localshot';
# - ALL PRIVILEGES(권한)
...
SELECT user, host FROM mysql.user WHERE user = 'readonly_use';
