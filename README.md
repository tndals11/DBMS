...
- **데이터 무결성 유지**  
  ex) 주문 후 결제 테이블 자동 삽입으로 데이터 오류 방지  
...
- **모든 트리거는 행 단위로 실행** (`FOR EACH ROW`)

---

### 🔹 트리거 실무 활용

- 특정 테이블 이벤트 발생 시 자동 로그 기록  
- 재고 수량 자동 조정  
- 포인트 자동 적립 등 자동화된 비즈니스 로직  

---

### 🔹 주의 사항

- 트리거 남용 시 디버깅 어려움 및 성능 저하 발생 가능  
- 필요한 곳에만 최소한으로 사용 권장
...
CREATE TRIGGER 트리거명
    트리거시점 (BEFORE | AFTER) 이벤트종류 (INSERT | UPDATE | DELETE)
    ON 테이블명
    FOR EACH ROW
BEGIN
    -- 실행할 SQL 코드 작성 --
