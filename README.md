...
**반정규화(De-normalization)**는 데이터베이스 설계 시 정규화(Normalization)된 구조를 **의도적으로 일부 해제**하여, **성능 향상**이나 **조회 단순화**를 도모하는 데이터 구조 설계 기법입니다.  
주로 **조회 위주 시스템**, **리포팅**, **대용량 읽기 작업**이 많은 경우에 사용됩니다.
...
- 시스템이 **조회(Read) 중심**이고, 쓰기(Write)보다 읽기 비율이 매우 높을 때
- **리포트/통계 기반 시스템**, 대시보드, 검색 최적화가 필요한 경우
- 정규화로 인해 **과도한 JOIN**이 성능 병목을 유발하는 경우
- 실시간 응답 속도가 중요한 경우 (예: 사용자 대시보드, 검색 페이지 등)

---

### ✅ 장점

- **조회 성능 향상**: 필요한 데이터를 한 테이블에서 바로 가져올 수 있어 응답 시간이 빨라짐
- **쿼리 단순화**: JOIN이 줄어들고 쿼리가 짧아짐 → 개발 및 유지보수 용이
- **캐싱 효율성 증가**: 데이터를 미리 결합해놓으므로 캐시 적중률 상승

---

### ⚠️ 단점

- **데이터 중복 발생**: 동일한 정보가 여러 테이블 혹은 레코드에 존재할 수 있음
- **데이터 무결성 유지 어려움**: 변경 시 모든 중복 데이터를 함께 수정해야 함
- **쓰기 성능 저하 가능성**: 업데이트/삭제 시 수정 범위가 넓어짐
...
→ 사용자의 이름을 조회하려면 `JOIN`이 필요함.

### 🔹 반정규화된 구조

| order_id | user_id | user_name | order_date |
|----------|---------|-----------|------------|
| 101      | 1       | Alice     | 2025-08-01 |

→ `JOIN` 없이 `user_name` 바로 조회 가능
...
여러 개의 SQL 명령어를 **하나의 묶음**으로 처리하여, 그 중 하나라도 실패할 경우 전체 작업을 취소하고 이전 상태로 되돌립니다.
...
- 여러 SQL 작업을 하나의 **원자적 단위**로 묶어 처리함으로써  
  **데이터의 일관성과 신뢰성**을 유지
- 특히 **은행 시스템**, **쇼핑몰 결제**, **재고 처리** 등에서 핵심적으로 사용됨
  예시: A → B 송금  
  A 계좌에서 출금 -> B 계좌로 입금
> 위 두 작업은 반드시 **함께 실행되거나, 함께 취소**되어야 합니다.  
> 하나만 성공하고 다른 하나가 실패하면 **데이터 불일치** 문제가 발생합니다.
...
| 속성 | 설명 |
|------|------|
| **A - Atomicity (원자성)** | 트랜잭션 내 모든 작업은 **전부 성공**하거나, **전부 실패**해야 함 *(All or Nothing)* |
| **C - Consistency (일관성)** | 트랜잭션 전후의 데이터베이스는 항상 **일관된 상태**여야 함 |
| **I - Isolation (고립성)** | 동시에 여러 트랜잭션이 실행되어도 **서로 간섭 없이 독립적**으로 수행되어야 함 |
| **D - Durability (지속성)** | 트랜잭션이 성공적으로 완료되면, 그 결과는 **영구적으로 보존**됨 (장애 발생 시에도 유지) |
